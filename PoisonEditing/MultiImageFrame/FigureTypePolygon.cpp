...
FigureTypePolygon::FigureTypePolygon(vector<QPoint> points_array_)
{
...
FigureTypePolygon::~FigureTypePolygon()
{
	y_max_ = y_min_ = 0;
	x_max_ = x_min_ = 0;
	m_vertices.clear();
...
void FigureTypePolygon::Get_Vertices()
{
	m_vertices.clear();
...
void FigureTypePolygon::Generate_BBox()
{
	BBox_ =  QRect(x_min_,y_min_,x_max_-x_min_+1,y_max_-y_min_+1);
}

void FigureTypePolygon::Get_Y_Max()
{
	int i,size_=Points_.size();
...
void FigureTypePolygon::Get_Y_Min()
{
	int i,size_=Points_.size();
...
void FigureTypePolygon::Get_X_Max()
{
	int i,size_=Points_.size();
...
void FigureTypePolygon::Get_X_Min()
{
	int i,size_=Points_.size();
...
void FigureTypePolygon::Get_Points_Num()
{
	points_num_ = Points_.size();
}

void FigureTypePolygon::Draw_Selected_Region_(QPainter &painter)
{
