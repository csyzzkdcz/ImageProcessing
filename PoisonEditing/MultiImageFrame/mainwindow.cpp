...
void MainWindow::CreateActions()
{
	...
	action_choose_rect_ = new QAction(QIcon(":/MainWindow/Resources/images/rect.png"), tr("RectChoose"), this);
	action_gray_->setStatusTip(tr("Choose rectangle region"));
	connect(action_choose_rect_, SIGNAL(triggered()), this, SLOT(ChooseRect()));

	action_choose_polygon_ = new QAction(QIcon(":/MainWindow/Resources/images/polygon.png"), tr("RectChoose"), this);
	action_gray_->setStatusTip(tr("Choose closed freehand region"));
	connect(action_choose_polygon_, SIGNAL(triggered()), this, SLOT(ChoosePolygon()));

	action_choose_freehand_ = new QAction(QIcon(":/MainWindow/Resources/images/freehand.png"), tr("RectChoose"), this);
	action_gray_->setStatusTip(tr("Choose closed freehand region"));
	connect(action_choose_freehand_, SIGNAL(triggered()), this, SLOT(ChooseFreehand()));

	action_copy_ = new QAction(QIcon(":/MainWindow/Resources/images/copy.png"), tr("Copy"), this);
	action_gray_->setStatusTip(tr("Copy image in the chosen region"));
	connect(action_copy_, SIGNAL(triggered()), this, SLOT(Copy()));

	action_paste_ = new QAction(QIcon(":/MainWindow/Resources/images/paste.png"), tr("Paste"), this);
	action_gray_->setStatusTip(tr("Paste rect region to object image"));
...
void MainWindow::CreateMenus()
{
	...
	menu_edit_->addAction(action_choose_rect_);
...
{
	// Let source child window prepare for pasting
	ChildWindow *child = qobject_cast<ChildWindow *>(mdi_area_->activeSubWindow()->widget());
	child->imagewidget_->Prepare();   // compute the LU solver in advance
	child->imagewidget_->is_copied_ = true;
}
