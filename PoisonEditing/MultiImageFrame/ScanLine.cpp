...
void ScanLine::Init_Figure(FigureType *figure)
{
	figure->Get_Vertices();
	figure->Get_Points_Num();
	figure->Get_X_Max();
	figure->Get_X_Min();
	figure->Get_Y_Max();
	figure->Get_Y_Min();
	figure->Generate_BBox();
	BBox = figure->BBox_;
	...
	for(i=0;i<size_;i++)
		my_vertices_.push_back(figure->m_vertices[i]);
	Init_ScanLine_Array();
	is_init_figure_ = true;
}

void ScanLine::InitNET(FigureType *figure)
{
	if(!is_init_figure_)
		Init_Figure(figure);
	MY_EDGE temp_edge_;
...
void ScanLine::Scan_Line_Algorithm(FigureType *figure)
{
	InitNET(figure);
	for(int i=y_min_;i<=y_max_;i++)
	{
