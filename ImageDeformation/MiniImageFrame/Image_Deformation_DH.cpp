...
Image_Deformation_DH::Image_Deformation_DH()
{
}
...
void Image_Deformation_DH::Deformation(cv::Mat image_mat_,cv::Mat &target_image_mat_,PolygonCage *poly_cage_,PolygonCage* target_poly_cage_)
// !!! Attention : This method can be only applied when the polygoncage is convex,otherwise the results are not as desired
{
	...
	for(i=target_poly_cage_->vertices_.size();i<triangle_frame_.numberofpoints;i++)
	{
		...
		{
			double cot_rj_ = calculate_cotangent(target_poly_cage_->vertices_[(j-1+target_poly_cage_->vertices_.size())%target_poly_cage_->vertices_.size()].rx(),target_poly_cage_->vertices_[(j-1+target_poly_cage_->vertices_.size())%target_poly_cage_->vertices_.size()].ry(),target_poly_cage_->vertices_[j].rx(),target_poly_cage_->vertices_[j].ry(),triangle_frame_.pointlist[2*i],triangle_frame_.pointlist[2*i+1]);
			double cot_bj_ = calculate_cotangent(target_poly_cage_->vertices_[(j+1)%target_poly_cage_->vertices_.size()].rx(),target_poly_cage_->vertices_[(j+1)%target_poly_cage_->vertices_.size()].ry(),target_poly_cage_->vertices_[j].rx(),target_poly_cage_->vertices_[j].ry(),triangle_frame_.pointlist[2*i],triangle_frame_.pointlist[2*i+1]);
			double weight_j_ = (cot_rj_+cot_bj_);
